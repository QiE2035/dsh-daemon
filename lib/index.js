...
// authoritative detector; the version gate only lets the watchdog skip the
// (harmless, 15s) poll when the installed dsh is KNOWN to predate token auth.
...
function apply(ctx) {
  ...
  if (sandboxHarness) {
    ...
    async function discover(portArg) {
      ...
      return { home, dshHome, nodePath, dshBin, port, dshVersion };
    }
    ...
    function watchdogScript(cfg, p, opts) {
      ...
      return '\'use strict\';\n' +
        ...
        'const DSH_PKG_JSON = PATH.join(PATH.dirname(DSH_BIN), \'..\', \'package.json\');\n' +
        'function dshVersion() {\n' +
        '  try { return JSON.parse(FS.readFileSync(DSH_PKG_JSON, \'utf8\')).version || null; } catch (e) { return null; }\n' +
        '}\n' +
        ...
        '// keeps the current behaviour: never touch the browser.\n' +
        ...
        'const WEB_TOKEN_POLL_MS = 15000;\n' +
        'const WEB_TOKEN_POLL_STEP_MS = 250;\n' +
        ...
        '    const wrapper = CP.spawn(\'powershell.exe\', [\'-NoProfile\', \'-NonInteractive\', \'-Command\', ps], { stdio: fd >= 0 ? [\'ignore\', fd, fd] : \'ignore\', env: env, windowsHide: true });\n' +
        ...
        '    })();\n' +
        '  } else {\n' +
        '    const child = CP.spawn(process.execPath, args, { detached: true, stdio: fd >= 0 ? [\'ignore\', fd, fd] : \'ignore\', env: env, windowsHide: true });\n' +
        ...
        '  const deadline = Date.now() + WEB_TOKEN_POLL_MS;\n' +
        '  (function poll() {\n' +
        '    let text = \'\';\n' +
        '    try {\n' +
        ...
        '      if (st.size > start) {\n' +
        '        const fd = FS.openSync(WEB_LOG, \'r\');\n' +
        ...
        '      else log(\'browser open disabled (DSH_DAEMON_OPEN_BROWSER=0) — open manually: \' + url);\n' +
        '      return;\n' +
        '    }\n' +
        '    if (Date.now() >= deadline) {\n' +
        '      webAuthPollActive = false;\n' +
        '      log(\'no web auth token line within \' + Math.round(WEB_TOKEN_POLL_MS / 1000) + \'s (old dsh or non-token web)\');\n' +
        '      return;\n' +
        '    }\n' +
        '    setTimeout(poll, WEB_TOKEN_POLL_STEP_MS);\n' +
        '  })();\n' +
        '}\n' +
        ...
        '      const c = CP.spawn(\'powershell.exe\', [\'-NoProfile\', \'-NonInteractive\', \'-Command\', cmd], { detached: true, stdio: \'ignore\', windowsHide: true });\n' +
...
if (typeof module !== 'undefined' && module.exports) {
  if (String(process.env.DSH_DAEMON_TEST_HOOK) === '1') {
    ...
    plugin.__test = { DSH_NO_OPEN_MIN, DSH_TOKEN_AUTH_MIN, parseSemver, preGt, versionGte, dshNoOpenSupported, dshTokenAuthSupported, extractTokenUrl };
  }
